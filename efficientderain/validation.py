...
if __name__ == "__main__":
    ...
    parser.add_argument('--load_name', type=str, default='./models/v4_rain1400/v4_rain1400.pth',
    ...
    for i, (true_input, true_target, height_origin, width_origin) in enumerate(test_loader):
        ...
        sample_name = '%d' % (i + 1)
        utils.save_sample_png(sample_folder=sample_folder, sample_name='%d' % (i + 1), img_list=img_list,
                              name_list=name_list, pixel_max_cnt=255, height=int(height_origin), width=int(width_origin))
